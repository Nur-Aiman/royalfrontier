...
// Sort the orders array by date (latest at the top) and by order status (pending at the top)
orders.sort((a, b) => {
...
if (dateA < dateB) return 1;
// Sort by order status (pending first)
const statusA = a.order_status.some(status => status === 'pending');
const statusB = b.order_status.some(status => status === 'pending');
